...
@Data
public class Token {
    @Schema(maxLength=2, minLength=2, required = true, description = "이 Token을 소유하는 MSP가 운영중인 국가의 ISO-3166 alpha-2 국가 코드입니다.", defaultValue="KR", example="KR")
    private String country_code;
    @Schema(maxLength=3, minLength=3, required = true, description = "이 Token을 소유하는 MSP의 ID로 ISO-15118 표준을 따릅니다.")
    private String party_id;
    @Schema(maxLength=36, required = true, description = "이 Token의 [TokenType]과 함께 식별할 수 있는 고유 ID 입니다.")
    private String uid;
    ...
    @Schema(maxLength=64, required=true, description = "발행 회사, 대부분의 경우 토큰(RFID 카드)에 인쇄된 회사 이름, 반드시 eMSP는 아닙니다.")
    ...
    @Schema(required=true, description = "이 토큰의 유효 여부입니다.")
    private boolean valid;
    @Schema(required=true, description = "허용되는 WhitelistType(화이트리스트 유형)을 나타냅니다.")
    ...
    private ProfileType default_profile_type;
