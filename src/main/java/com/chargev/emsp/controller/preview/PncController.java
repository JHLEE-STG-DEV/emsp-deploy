...
public class PncController {
    ...
    public PncApiResponseObject issueCheck(HttpServletRequest httpRequest, @RequestBody CheckRequest request) {
        ...
        //ServiceResult<String> result = certificateService.issueContract(request, trackId);
        ...
        //     ContractStatus data = result.getData().orElse(null); // Optional에서 값을 추출
        ...
        response.setMessage("Not Finished.");
        ContractStatus sampleData = new ContractStatus();
        sampleData.setStatus(1);
        ContractInfo sampleInfo = new ContractInfo();
        sampleInfo.setEmaid("KRCEVCA1000000");
        
        logService.controllerLogEnd(trackId,true, response.getCode(), response.getData().orElse(null));
        return response;
    }
    ...
    public PncApiResponse revokeContract(HttpServletRequest httpRequest,@RequestBody PncReqBodyRevokeContractCert request) {
        // 요청 URL 가져오기
        String requestUrl = httpRequest.getRequestURL().toString();
    ...
    public PncApiResponseObject getContractInfo(HttpServletRequest httpRequest,@RequestBody PncReqBodyContractInfo request) {
        // 요청 URL 가져오기
        String requestUrl = httpRequest.getRequestURL().toString();
